...
void NormalDepthMapInit(ID3D11RenderTargetView** RTV, ID3D11ShaderResourceView** SRV, ID3D11DepthStencilView** DSV)
{
	...
	aoND_desc.Width = SSAO_RES_X;
	aoND_desc.Height = SSAO_RES_Y;
	aoND_desc.MipLevels = 1;
	aoND_desc.MiscFlags = 0;
	aoND_desc.SampleDesc = { 1,0 };
	aoND_desc.Usage = D3D11_USAGE_DEFAULT;
	ComPtr<ID3D11Texture2D> aoNDMap;
	r_assert(
		DX_Device->CreateTexture2D(&aoND_desc, nullptr, aoNDMap.GetAddressOf())
	);
	...
	r_assert(
		DX_Device->CreateRenderTargetView(aoNDMap.Get(), &aoNDRTV_desc, RTV)
	);
	...
	r_assert(
		DX_Device->CreateShaderResourceView(aoNDMap.Get(), &aoNDSRV_desc, SRV)
	);
...
void SSAO::Update(const Camera* camera, const std::vector<Object*>& objs)
{
	...
	DrawNormalDepth(camera, objs);
	DrawAccess(camera->ProjMat(Z_ORDER_STANDARD));
	BlurHorizon();
	BlurVertical();

	DX_DContext->OMSetRenderTargets(1, &oriPassRTV, oriDSV);
	DX_DContext->PSSetShaderResources(3, 1, finalSRV.GetAddressOf());
	DX_DContext->RSSetViewports(1, &oriVP);
}


void SSAO::DrawNormalDepth(const Camera* camera, const std::vector<Object*>& objs)
{
	...
	for (auto obj : objs)
	{
		// no render priority appied for ao
		assert(obj->zOrder == Z_ORDER_STANDARD);

		XMMATRIX w = obj->transform->WorldMatrix();
		XMMATRIX drawTransf[4] = {
			w,
			camera->VMat(),
			camera->ProjMat(Z_ORDER_STANDARD),
...
void SSAO::DrawAccess(const XMMATRIX& p)
{
	ID3D11ShaderResourceView* nullSRV = nullptr;
